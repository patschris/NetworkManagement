...
public class Authentication extends JFrame {
	...
	public Authentication(Connection c,Statement s) {
	...
	/*
	 Kratame kai emfanizoume ta access points pou de xrisimopoioun WPA2 gia authentication
	 dioti den einai asfalh
	 */ 
		String onScreen = "For maximum security, a network should include only devices with the latest in security \n"
				+ "technology – Wi-Fi Protected Access 2 (WPA2).\n \n";
		ResultSet rs=null;
		onScreen+="Following acess points haven't proper authentication:\n";
		textArea.setText(onScreen);
		try {
			rs = stmt.executeQuery("select ssid,defaultAuth from XirrusWifiInspector where defaultAuth not like '%WPA2%'");
			while (rs.next()){
				onScreen+= "> "+rs.getString("ssid")+" ("+rs.getString("defaultAuth")+")\n";
				textArea.setText(onScreen);
			}
			rs.close();
		}
