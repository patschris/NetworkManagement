...
public class PacketAnalysis extends JFrame {
	...
	public PacketAnalysis(Connection c,Statement s) {
		super("Packet Analysis");
		this.mainFrame = this;
		this.conn = c;
		this.stmt = s;
	...
	private void addTitle() {
		...
		JLabel title = new JLabel("Packet Analysis");
	...
	private void analyze() {
		...
		ResultSet rs = null;		
		try {
			rs = stmt.executeQuery("select count(protocol) as counter from Wireshark;");
			/*Pernoume to plithos olwn twn paketwn */
			while (rs.next()){
				onScreen+="Transfered "+rs.getInt("counter")+" packets \n";
			}
			rs.close();
			/*
			 Pairnoume ola ta diaforetika protokolla kai to plithos twn paketwn autou tou protokollou 
			 */
			rs = stmt.executeQuery("select protocol, count(protocol) as count from Wireshark group by protocol order by protocol");
			while (rs.next()) {
				onScreen+="\n"+rs.getString("protocol")+" : " + rs.getInt("count")+" packet(s)\n";
			}
			rs.close();
		} 
